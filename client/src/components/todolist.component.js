...
export default class ToDoList extends Component {
    ...
    render() {
      ...
      const listItems = this.state.todos.map((obj, i) =>
        ...
        <div id="checkbox" className="custom-cir">
          <input type="checkbox" id={`checkbox${i}`} checked={!!this.state.todos[i].isCompleted} onChange={e => this.onCompleteChange(e, i)}/>
          <label htmlFor={`checkbox${i}`}></label>
        </div>
        <p id="expM">{'!'.repeat(obj.priority)}</p>
        <p data-toggle="tooltip" className="text-truncate" data-placement="right" title={obj.description} id="itemTitle">{obj.title}</p> 
        </li>
      );
